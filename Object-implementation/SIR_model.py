...
def SIR_model(iters, xSize, ySize, beta, gamma, lambd, my, m, N):
    ...
    def susStep(S, I, beta, lambd, my, N, diff, h):
        return S + ((lambd*S) - (my*S) - (beta*S*I)/N - (diff*S)/N)*h

    def infStep(S, I, beta, gamma, my, N, diff, h):
        return I + ((beta*S*I)/N + (diff*S)/N - gamma*I - my*I)*h

    def remStep(I, R, gamma, my, h):
        return R + (gamma*I - my*R)*h

    def timeStep(susMat, infMat, remMat, beta, gamma, lambd, my, N, m, h):
        ...
        tempSus = susStep(susMat, infMat, beta, lambd, my, N, diff, h)
        tempInf = infStep(susMat, infMat, beta, gamma, my, N, diff, h)
        tempRem = remStep(infMat, remMat, gamma, my, h)
    ...
    for i in range(iters-1):
        tempSus, tempInf, tempRem = timeStep(susMat[:,:,i], infMat[:,:,i], remMat[:,:,i], beta, gamma, lambd, my, N, m, h)
