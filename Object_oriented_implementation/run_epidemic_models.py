...
class run_epidemic_models:
    ...
    #SIRS model with a seasonality factor of spread(omega)
    def SEIRSs(self, xSize, ySize, inf, alpha, beta, gamma, lambd, my, delta, omega, m, N, h=0.01):
        N = np.ones((xSize, ySize)) * N
        susMat, expMat, infMat, remMat = SEIRSs_m.SEIRSs_model(self.iters, xSize, ySize, alpha, beta, gamma, lambd, my, delta, omega, m, N, inf, h)

        return susMat, expMat, infMat, remMat

    #Test SIC for initial infected. Mobility factor set to zero.
    def SIC_test_inf(self, xSize, ySize, beta, gamma, lambd, my, delta, omega, N, inf1, inf2, alpha=0, h = 0.01):
        inf = np.linspace(inf1, inf2, (xSize*ySize))
        inf = np.reshape(inf, (xSize, ySize))
        if alpha == 0:
            susMat, infMat, remMat = SIRSs_m.SIRSs_model(self.iters, xSize, ySize, beta, gamma, lambd, my, delta, omega, 0, N, inf, h)
            return susMat, infMat, remMat, inf
        else:
            susMat, expMat, infMat, remMat = SEIRSs_m.SEIRSs_model(self.iters, xSize, ySize, alpha, beta, gamma, lambd, my, delta, omega, 0, N, inf, h)
            return susMat, expMat, infMat, remMat, inf

    #Test parameter sensitivty for population. Mobility factor set to zero.
    def param_seni_test_N(self, xSize, ySize, inf, beta, gamma, lambd, my, delta, omega, N1, N2, alpha=0, h=0.01):
    ...
    #Test parameter sensitivty for population. Mobility factor set to zero.
    def param_seni_test(self, xSize, ySize, inf, beta, gamma, lambd, my, delta, omega, N, para1, para2, paraChoise, alpha=0, h=0.01):
        ...
        if paraChoise == 1:
            ...
            print("Please choose parameter: 1 = beta, 2 = gamma, 3 = lambd, 4 = my, 5 = delta, 6 = omega, 7 = alpha")
            return
        if alpha.all() == 0:
            susMat, infMat, remMat = SIRSs_m.SIRSs_model(self.iters, xSize, ySize, beta, gamma, lambd, my, delta, omega, 0, N, inf, h)
    ...
    #Test parameter sensitivty for population. Mobility factor set to zero.
    def param_seni_test_m(self, xSize, ySize, inf, beta, gamma, lambd, my, delta, omega, N, para1, para2, alpha=0, h=0.01):
        ...
        if alpha.all() == 0:
            ...
            expRet = np.zeros((xSize, ySize, self.iters))
            for i in range(xSize):
                for j in range(ySize):
                    susMat, expMat, infMat, remMat = SEIRSs_m.SEIRSs_model(self.iters, xSize, ySize, alpha, beta, gamma, lambd, my, delta, omega, m[i,j], N, inf, h)
                    susRet[i,j,:] = np.sum(np.sum(susMat, axis=0), axis=0)
                    expRet[i, j,:] = np.sum(np.sum(expMat, axis=0), axis=0)
                    infRet[i, j,:] = np.sum(np.sum(infMat, axis=0), axis=0)
                    remRet[i, j,:] = np.sum(np.sum(remMat, axis=0), axis=0)

            return susRet, expMat, infRet, remRet, m


    #Test parameter sensitivty for population. Mobility factor set to zero.
    def param_seni_test_h(self, xSize, ySize, inf, beta, gamma, lambd, my, delta, omega, N, para1, para2, alpha=0):
        ...
        if alpha.all() == 0:
            ...
            return susMat, expMat, infMat, remMat, h
